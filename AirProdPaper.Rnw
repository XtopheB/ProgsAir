...
\subsection{Descriptive statistics}
 The number of complete observations is of \textbf{\Sexpr{nrow(unique(subset(Mydata, year == annee2, select= c(carrier))))}} carriers for \Sexpr{annee2}. Below is a partial overview of the file's production variables for those \textbf{fully} observable airlines those 2 years.
...
alphafrontier.2d(as.matrix(dataref$K),as.matrix(dataref$Y),type="input",alpha=alpha,
                 col='green',lty=1,add=TRUE, confidence = F, shade=F)
#FDH
...
dea.plot.frontier(couple.o[,1],couple.o[,2], RTS="fdh", add= TRUE, 
                  lty="dashed", col="purple", lwd=1)
# points(couple.o[,1],couple.o[,2], type="l", xlab="Xvol",  col="purple", pch="*", lty=2)

points(datalcc$K,datalcc$Y,  pch=19, col="red")
textxy(dataref$K,dataref$Y, dataref$carrier)
rug(dataref$K, col = "grey")

legend("topleft",legend=c( "FDH","alpha-frontier", "m-frontier", "LCC"),lty=c(2,1,2,9),col=c(  "blue","green", "purple", "red"))
@
...
#Alpha
alphafrontier.2d(as.matrix(dataref$K),as.matrix(dataref$Y),type="input",alpha=alpha,col='green',lty=1,add=TRUE, confidence = F, shade=F)
#FDH
dea.plot.frontier(dataref$K, dataref$Y, RTS="fdh", add= TRUE, 
                  lty="dashed", col="blue", lwd=1)
...
Zoom under the \Sexpr{BorneSup} quantile for year \Sexpr{annee} ...
...
#DEA ONE 1 INPUT  
dataref1 <- subset(Workdata,year==annee )  # <- QUE 2002
...
dataref1$DEA1 <-round(1/eff(spam), 3)

spam1 <- Benchmarking::dea(X=xref, Y=yref, RTS="fdh", ORIENTATION="in")
dataref1$FDH1 <-round(1/eff(spam1), 3)
...
spam <- Benchmarking::dea(X=xref, Y=yref, RTS="vrs", ORIENTATION="in")
dataref2$DEA1 <-round(1/eff(spam), 3)

spam1 <- Benchmarking::dea(X=xref, Y=yref, RTS="fdh", ORIENTATION="in")
dataref2$FDH1 <-round(1/eff(spam1), 3)
...
\subsection{Contemporaneous efficiencies (1 or 4 inputs !)}
% IL Y a 3 changements ` faire : 1 ici et 2 pour la Pooled frontiere (1 pour xref, 1 pour xobs)
<<echo=FALSE, results=hide>>=
