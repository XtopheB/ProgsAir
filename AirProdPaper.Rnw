...
% 19/08/2013 : Hyperbolic direction  
% 26/08/2013 : distinction small/big firms...

\documentclass[a4paper]{article}

\title{Airlines Efficiency with $\alpha$ and $order-m$ frontiers :\\
Non log and hyperbolic orientation}
\author{Christophe }
...
We restrict the file to observation which satisfy $K>0$ and $Y>0$; On this file we have \textbf{\Sexpr{length(unique(Mydata$carrier))}} different carriers and \textbf{\Sexpr{nrow(Mydata)}} observations.\\

% We should emphasize that only \textbf{\Sexpr{nrow(unique(subset(Mydata, Test0 >0, select= c(carrier))))}} carriers  

% Table des effectifs

<<echo=FALSE, results=hide, label=EffectifsNEW>>=
## Definition de la Table
table.data<-data.frame(toto=numeric(0), toto=numeric(0), toto=numeric(0), toto=numeric(0), toto=numeric(0)) 

Mydata.all <- data.frame(dataall)
x=seq(2002, 2008, by=1)  
nbtemp=length(x)

for (i in 1:length(x)) {
  
n1 <- nrow(subset(Mydata.all, Mydata.all$year == x[i]))
n2 <- nrow(subset(Mydata.all, Mydata.all$year == x[i] & Mydata.all$K>0 & Mydata.all$Y>0))
n4 <- nrow(subset(Mydata.all, Mydata.all$year == x[i] & Mydata.all$Test0>0 ))
nloss <- (1-n4/n2)*100  
    spam <-cbind(x[i],n1, n2, n4, nloss) 
    table.data <-rbind(table.data,spam)
} 
@

<<echo =FALSE, results = tex, label=TableCount>>=
# mise en forme du tableau
names(table.data) <- c("Year",  "N.Complete", "N.2inputs", "N.4inputs", "Perc.loss")
foo <- xtable(table.data, label="TableCount",
                caption =paste("The total nb of obs. is",nrow(Mydata.all),", for",
                             nrow(unique(subset(Mydata.all, select= c(carrier)))),
                               "unique and complete airlines (without selection)."), align = "ll|cccc")
digits(foo)[c(2:5)] <- 0
digits(foo)[6] <- 2
...
We decide to work in the \textbf{hyperbolic-oriented} framework
...
# cas hyper-oriented
couple.h <-cbind(dataref$K*scoreM$hyper, dataref$Y/scoreM$hyper)
couple.h <- couple.h[order( couple.h[,1],couple.h[,2]),]

# Plots 
plot(dataref$K,dataref$Y,  pch="+", col="darkblue",
      ylab="Y", xlab= "K",
     main=paste(" Frontiers (hyper-oriented) in", annee),
...
#Alpha-Frontier
alphafrontier.2d(as.matrix(dataref$K),as.matrix(dataref$Y),type="hyper",alpha=alpha,
                 col='green',lty=1,lwd=2, add=TRUE, confidence = F, shade=F)


#Order-m frontier ( On "lisse" avec le FDH des points g?n?r?s sur la m-fronti?re)
dea.plot.frontier(couple.h[,1],couple.h[,2], RTS="fdh", add= TRUE, 
...
\clearpage
\subsection{Graphical comparison input-output-hyper}
...
# Plots 
plot(dataref$K,dataref$Y,  pch="+", col="darkgrey",
       #xlim=c(0,bornex),ylim=c(0,borney),
      ylab="Y", xlab= "K",
     main=paste(" Alpha Frontiers (all-orientations) in", annee),
     sub=paste(nrow(dataref),"Airlines (alpha=",alpha,")" ) )
...
alphafrontier.2d(as.matrix(dataref$K),as.matrix(dataref$Y),type="hyper",alpha=alpha,
                 col='blue',lty=1,lwd=2, add=TRUE, confidence = F, shade=F)
...
legend("topleft",legend=c( "FDH","alpha (input)", "alpha (output)", "alpha (hyper)"),lty=c(2,1,1,1),col=c(  "blue","green", "brown", "blue"))
@
...
plot(dataref$K,dataref$Y,  pch="+", col="darkgrey",
      ylab="Y", xlab= "K",
     main=paste(" Order-m Frontiers (all-orientations) in", annee),
     sub=paste(nrow(dataref),"Airlines (alpha=",alpha," M=",M.order,")" )
)
#FDH
dea.plot.frontier(dataref$K, dataref$Y, RTS="fdh", add= TRUE, 
...
#points(couple.i[,1],couple.i[,2], type="l", xlab="Xvol",  col="green", pch="*", lty=2)
...
#points(couple.h[,1],couple.h[,2], type="l", xlab="Xvol",  col="blue", pch="*", lty=2)

legend("topleft",legend=c( "FDH","order-m (input)", "order-m (output)", "order-m (hyper)"),
       lty=c(2,1,1,1),col=c(  "blue","green", "brown", "blue"))
@

\clearpage
\subsection{Graphical comparison of $\alpha$-frontier for various $\alpha$s}
...
plot(dataref$K,dataref$Y,  pch="+", col="darkblue",
      ylab="Y", xlab= "K",
     main=paste(" Alpha Frontiers (hyper-oriented), various alphas in", annee),
     sub=paste(nrow(dataref),"Airlines (various alpha, hyper oriented, M=",M.order,")" )
)
a=seq(from=0.90, to=1, by=0.02)
colors <- rainbow(length(a)) 
for (i in 1:length(a)) {
#Alpha-Frontier Hyper
alphafrontier.2d(as.matrix(dataref$K),as.matrix(dataref$Y),type="hyper",
...
legend("topleft", legend=c(a), col=rainbow(length(a)), lwd = 1, lty = 1, title="Alpha")
...
#DEA ONE 1 INPUT  
dataref1 <- subset(Workdata,year==annee )  # <- select year
...
# M-frontier -  Scores hyper-oriented 

spam2 <- ordermscore(xref, yref, xeval=xref, yeval=yref, m=M.order)
dataref1$Mscore1 <- round(spam2$hyper, 3) 

#alpha-frontiere

foo <- alphascore(xref, yref, xeval=xref, yeval=yref, alpha=alpha)
dataref1$Ascore1 <-round(foo$hyper, 3)

#Table
Mycap <- paste("Efficiency scores for",annee,", m=",M.order,", alpha = ", 
               alpha,"(hyper-oriented, K as unique input).")

Mylab <- "Monlabel"
Myvars<- with(dataref1, data.frame( "DEA(vrs-Input)" = dataref1$DEA1,
                                      "FDH(input)" = dataref1$FDH1,
...
# M-frontier 
# Scores hyper-oriented 

spam2 <- ordermscore(xref, yref, xeval=xref, yeval=yref, m=M.order)
#dataref2$Mscore1 <- round(spam2$input, 3) 
dataref2$Mscore1 <- round(spam2$hyper, 3) 


#alpha-frontiere
foo <- alphascore(xref, yref, xeval=xref, yeval=yref, alpha=alpha)
dataref2$Ascore1 <- round(foo$hyper, 3)

#Table
Mycap <- paste("Efficiency scores for",annee,", m=",M.order,", alpha = ", alpha,"(hyper-oriented, K as unique input).")
...
%\newpage
%\clearpage
\section{Detailled airlines efficiency scores}

\subsection{\textbf{Big} airlines efficiency ranking, year \Sexpr{annee1}}

<<echo =FALSE, results = hide,  label=ThresholdBig1>>=
# definition of BIG  
threshold <- .50
biglimit <- quantile(dataref1$Y,probs=threshold)

nlist <- 20  # length of the table...
# mise en forme du tableau ONE INPUT 
spam <- data.frame(dataref1$carrier, dataref1$carriername, dataref1$DEA1,
              dataref1$FDH1, dataref1$Mscore1, dataref1$Ascore1, dataref1$Y)

@
We define Big airlines as those being above the quantile \Sexpr{threshold} for the output variable $Y$. Here, for year \Sexpr{annee1} the lower bound of $Y$ is then \Sexpr{round(biglimit, 2)}.

<<echo =FALSE, results = tex, label=TabScoresBestBigYear1>>=
spam.big <- subset(spam, dataref1$Y > biglimit )

spam.big <- spam.big[order(spam.big[, 6], decreasing =TRUE), ]

foo1.big <- xtable(spam.big[1:nlist,1:7], digits=2, label ="Eff", 
                   caption = paste("",nlist,"BEST BIG airlines efficiency (sorted by alpha-scores),
                                   year",annee1, ", m=",M.order,", alpha = ", alpha,
                                   "(hyper oriented, 1 input),"
                                   ,nrow(dataref1),"airlines. "))

names(foo1.big) <- c("code", "Airline", "DEA", "FDH", "M-Score", "alpha-Score", "Y")
print(foo1.big, include.colnames=TRUE, include.rownames=FALSE  )
@


<<echo =FALSE, results = tex, label=TabScoresWorseBigYear1>>=

spam.big.W <- spam.big[order(spam.big[, 6], decreasing =FALSE), ] # <<<-----  Sort   !

foo1.big.W <- xtable(spam.big.W[1:nlist,1:7], digits=2, label ="Eff", 
                   caption = paste("",nlist,"WORSE BIG airlines efficiency (sorted by alpha-scores),
                                   year",annee1, ", m=",M.order,", alpha = ", alpha,
                                   "(hyper oriented, 1 input),"
                                   ,nrow(dataref1),"airlines. "))

names(foo1.big.W) <- c("code", "Airline", "DEA", "FDH", "M-Score", "alpha-Score", "Y")
print(foo1.big.W, include.colnames=TRUE, include.rownames=FALSE  )
@

\clearpage

\subsection{\textbf{Small} airlines efficiency ranking, year \Sexpr{annee1}}


We define \textbf{small} airlines as those being below the quantile \Sexpr{threshold} for the output variable $Y$. Here, for year \Sexpr{annee1} the lower bound of $Y$ is then \Sexpr{round(biglimit, 2)}.

<<echo =FALSE, results = tex, label=TabScoresBestSmallYear1>>=
spam.small <- subset(spam, dataref1$Y < biglimit )

spam.small <- spam.small[order(spam.small[, 6], decreasing =TRUE), ]

foo1.small <- xtable(spam.small[1:nlist,1:7], digits=2, label ="Eff", 
                   caption = paste("",nlist,"BEST SMALL airlines efficiency (sorted by alpha-scores),
                                   year",annee1, ", m=",M.order,", alpha = ", alpha,
                                   "(hyper oriented, 1 input),"
                                   ,nrow(dataref1),"airlines. "))

names(foo1.small) <- c("code", "Airline", "DEA", "FDH", "M-Score", "alpha-Score", "Y")
print(foo1.small, include.colnames=TRUE, include.rownames=FALSE  )
@


<<echo =FALSE, results = tex, label=TabScoresWorseSmallYear1>>=

spam.small.W <- spam.small[order(spam.small[, 6], decreasing =FALSE), ] # <<<-----  Sort   !

foo1.small.W <- xtable(spam.small.W[1:nlist,1:7], digits=2, label ="Eff", 
                   caption = paste("",nlist,"WORSE SMALL airlines efficiency (sorted by alpha-scores),
                                   year",annee1, ", m=",M.order,", alpha = ", alpha,
                                   "(hyper oriented, 1 input),"
                                   ,nrow(dataref1),"airlines. "))

names(foo1.small.W) <- c("code", "Airline", "DEA", "FDH", "M-Score", "alpha-Score", "Y")
print(foo1.small.W, include.colnames=TRUE, include.rownames=FALSE  )
@


\clearpage

\subsection{\textbf{All} airlines efficiency ranking, year \Sexpr{annee1}}

<<echo =FALSE, results = tex, label=TabScores1>>=

spam <- spam[order(spam[, 6], decreasing =TRUE), ]

foo2 <- xtable(spam[1:nlist,1:7], digits=2, label ="Eff", caption = paste("",nlist,"first airlines efficiency, year",annee1,", m=",M.order,", alpha = ", alpha,"(hyper oriented, 1 input), ",nrow(dataref1),"airlines (sorted by alpha-scores)."))

names(foo2) <- c("code", "Airline", "DEA", "FDH", "M-Score", "alpha-Score", "Y")
print(foo2, include.colnames=TRUE, include.rownames=FALSE  )
@

\clearpage
\subsection{\textbf{All} airlines efficiency ranking, year \Sexpr{annee2}}
...
spam <- data.frame(dataref2$carrier, dataref2$carriername, dataref2$DEA, 
                   dataref2$FDH,  dataref2$Mscore, dataref2$Ascore,dataref2$Y )

spam <- spam[order(spam[, 6], decreasing =TRUE), ]

foo2 <- xtable(spam[1:nlist,1:7], digits=2, label ="Eff", 
               caption = paste("", nlist,"first airlines efficiency, year",annee2,", 
                            m=",M.order,", alpha = ", alpha,"(hyper oriented, 1 input), 
...
# recuperation du m score  TOUTES  firmes sur frontiere contemporaine 
    ...
    score.m.cont <- c(score.m.cont , spam.c$hyper) 
...
# rC)cupC)ration du m score  TOUTES  firmes sur frontiere contemporaine
    ...
    score.alpha.cont <- c(score.alpha.cont , eggs.c$hyper) 
...
boxplot(score.m.cont[,2]~score.m.cont[,1], 
        ...
        col=colors, 
        sub = paste(" Hyper-oriented. 2002-2008, with",Ninput,"inputs, m=", M.order,"."), 
        xlab = "Years ", outline =FALSE)
abline(h = 1, col = "red",  lwd=2)
@
...
boxplot(score.alpha.cont[,2]~score.alpha.cont[,1], 
        ...
        col=colors, 
         sub = paste("Hyper-oriented. 2002-2008, with",Ninput,"inputs, alpha =",alpha,"."),  
        ylab = "Efficiency", 
        xlab = "Years ", outline =FALSE)
abline(h = 1, col = "red",  lwd=2)
@
...
for (i in 1:length(x)) {
    ...
    score.year.year <-score.farell$eff
    ...
    foo <- foo[order(foo[,3], decreasing=TRUE),]
    spam <- spam[order(spam[, 6], decreasing =TRUE), ]
...
# recuperation du m score  TOUTES  firmes sur frontiere complete (avec toutes les firmes)  
    ...
    score.m.all <- c(score.m.all,spam$hyper) 
    
#alpha-frontiere
# recuperation du m score  TOUTES  firmes sur frontiere complete (avec toutes les firmes)  
    eggs <- alphascore(xref, yref, xeval=xobs, yeval=yobs,alpha=alpha)
    score.alpha.all <- c(score.alpha.all,eggs$hyper) 

    
    #Definition des annees  
    spoof <- rep(x[i], nobs)
    y <- c(y, spoof)
 }
...
boxplot(score.dea[,2]~score.dea[,1], 
        #ylim=c(0,1), 
        main = "Airlines efficiency on DEA-Metafrontier", 
        col=colors, 
...
<< echo=FALSE, results=hide, fig=FALSE, label=QQplotDEA>>=
...
boxplot(score.fdh[,2]~score.fdh[,1], 
        col=colors, 
        main = "Airlines efficiency  on FDH Meta-frontier", 
...
<< echo=FALSE, results=hide, fig=FALSE, label=QQplotFDH>>=
...
boxplot(score.m[,2]~score.m[,1], 
        ...
        ylab = "Efficiency (hyper oriented)", 
        xlab = "Years ", outline =FALSE)
abline(h = 1, col = "red",  lwd=2)
...
%Other representations : Suppressed (30/07/2013)

<<echo=FALSE, results=hide, fig=FALSE, label=QQDensity>>=
