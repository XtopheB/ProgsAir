...
We compute below the $\alpha$-frontier with only one input ($K$, here) and one output and graph it for \Sexpr{annee1} and \Sexpr{annee2}. As soon as we have and aggregate input index, we'll replace it here...
...
dea.plot.frontier(dataref.y$K, dataref.y$Y, RTS="vrs", add= TRUE, lty="dashed", col="blue", lwd=1)
