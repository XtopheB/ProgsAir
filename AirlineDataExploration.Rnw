...
Workdata <-subset(Mydata, Test0 > 0)   #<<<< ---- Workdata = tout l'echantillon sauf les erreurs
...
We should emphasize that only \textbf{\Sexpr{nrow(unique(subset(Mydata, Test0 >0, select= c(carrier))))}} carriers  have \textbf{all} production variables (i.e. Y, K, L E \& M not null)  at least one year (based on "Test0" variable).
...
We choose \Sexpr{annee1} and  \Sexpr{annee2} to be 2 years of reference in our analysis. The number of complete observations is of \textbf{\Sexpr{nrow(unique(subset(Mydata, Test0 >0 & year == annee2, select= c(carrier))))}} carriers for \Sexpr{annee2}. Below is a partial overview of the file's production variables for those \textbf{fully} observable airlines those 2 years.
...
\section{Exploration of major airlines}
...
attach(Workdata)
hist(Y,  col="lightblue", border="pink", freq = TRUE)
#title(main="Distribution of output and input variables"))
@


<<fig=TRUE,echo=FALSE>>=
op <- par(mfrow = c(2, 2)) # 4 x 1 pictures on one plot
hist(K,  col="lightblue", border="pink", freq = TRUE)
#title(main="Distribution of output and input variables"))
hist(L,  col="lightblue", border="pink", freq = TRUE)
hist(E,  col="lightblue", border="pink", freq = TRUE)
hist(M,  col="lightblue", border="pink", freq = TRUE)
par(op)   
detach(Workdata)
@
...
Calcul des scores relatifs C  l'une ou l'autre des frontiC(res : 
...
# rC)cupC)ration des rC)sultats Firmes big sur frontiere restreinte big
    score.farell <- Benchmarking::dea(X=xobs, Y=yobs, RTS="vrs", ORIENTATION="out")
    score.big.big <-1/score.farell$eff

    
# rC)cupC)ration des rC)sultats Firmes big sur frontiere complete (avec toutes les firmes)  
...
boxplot(score.big.big , score.big.all, col="lightblue",
        main="Major's efficiency distribution relative to two frontiers",
        outline = FALSE,
...
plot(dataref.y$E,dataref.y$Y,  pch="+", col="darkblue",
     main=paste(" alpha-frontier (output-oriented) in", annee, ", alpha=",alpha,"") )
...
legend("bottomright",legend=c("Points", "DEA VRS","alpha-frontier", "confidence bound"),lty=1:4,col=c( "black", "blue","green", "grey"))
@
...
plot(dataref.y$E,dataref.y$Y,  pch="+", col="darkblue", 
     main=paste(" alpha-frontier alpha-frontier (output-oriented)in", annee, ", alpha=",alpha,"") )
textxy(dataref.y$E,dataref.y$Y, dataref.y$carrier)
...
foo2 <- xtable(spam[,], digits=2, label ="Eff", caption = paste("Airlines efficiency, year",annee1,", m=",M.order,", alpha = ", alpha,"(output oriented, 4 inputs), ",nrow(dataref),"airlines."))
...
foo2 <- xtable(spam[,], digits=2, label ="Eff", 
               caption = paste("Airlines efficiency, year",annee2,", 
                            m=",M.order,", alpha = ", alpha,"(output oriented, 4 inputs), 
                               ",nrow(dataref),"airlines."))
...
y.c <- NA               # annC)es repliquC)es sur les observations
...
# rC)cupC)ration du score DEA TOUTES  firmes sur frontiere contemporaine
...
boxplot(score.dea.cont[,2]~score.dea.cont[,1], 
        main = "Airlines efficiency measured to contemporaneous frontier", 
        sub = paste(" DEA frontier computed on all available airlines (2002-2008)"),  
...
% Efficiencies computed relative to pooled frontier 
<<echo=FALSE, results=hide>>=
...
y <- NA               # annC)es rpliquC)es sur les observations

# Frontier defined for all years does not change  #

dataref <- subset(Workdata)  # <- frontiere toutes annC)es
nref=nrow(dataref)
xref=matrix(c(dataref$K,dataref$L,dataref$E,dataref$M),nrow=nref,ncol=4)
yref=matrix(c(dataref$Y),nrow=nref,ncol=1)
...
# rC)cupC)ration du score DEA TOUTES  firmes sur frontiere complete (avec toutes les firmes)  
...
# rC)cupC)ration du m score  TOUTES  firmes sur frontiere complete (avec toutes les firmes)  
...
boxplot(score.dea[,2]~score.dea[,1], 
        main = "Airlines efficiency measured to pooled frontier", 
        sub = paste(" DEA frontier computed on ", nref, "airlines (2002-2008)"),  
        ylab = "Efficiency (output oriented)", 
        xlab = "Years ", outline =FALSE)
abline(h = 1, col = "red",  lwd=2)
...
% << echo=FALSE, results=hide, fig=TRUE>>=
% qplot(y, score.dea.all, data=score.dea, geom=c("boxplot", "jitter"), 
%       fill=y, main="Airlines efficiency measured to pooled frontier",
...
% 
% qplot(score.dea.all ,data=score.dea, geom="density", fill=y, alpha=I(.5), 
%    main="Distribution of efficiencies over time", xlab="DEA-score efficiency relative to pooled frontier ", 
%    ylab="Density")
% 
% @
...
boxplot(score.m[,2]~score.m[,1], 
        main = "Airlines m-efficiency  measured to pooled frontier", 
...
boxplot(score.alpha[,2]~score.alpha[,1], 
        main = "Airlines alpha-efficiency measured to pooled frontier", 
...
% Essai de graphique reliant les firmes par rang  
% <<echo=FALSE, results=hide, fig=TRUE >>=
% annee =2006
% # Calcul des efficacitis avec DEA, m-frontieres et alpha-frontieres 
% #DEA
% 
% dataref.y1 <- subset(Workdata,year==annee )  # <- QU'Une annC)e
% nref.y1=nrow(dataref.y1)
% 
% xref=matrix(c(dataref.y1$K,dataref.y1$L,dataref.y1$M),nrow=nref.y1,ncol=3)
% yref=matrix(c(dataref.y1$Y),nrow=nref.y1,ncol=1)
...
% dataref.y1$Ascore <-round(foo$output, 3)
% 
% foo <- data.frame("Name" = dataref.y1$carriername, "Y"= dataref.y1$Y,  "DEA"= dataref.y1$DEA, "Mscore"= dataref.y1$Mscore, "Ascore"=dataref.y1$Ascore)
% 
% #tri des donnC)es 
% foo <-  foo[with(foo, order(-DEA, -Y)), ]
% # CrC)ation des rankings DEA
% foo <-transform(foo,  dea.rank = rank(-DEA, ties.method = "first"))
% 
% #tri des donnC)es 
% foo <-  foo[with(foo, order(-Mscore, -Y)), ]
% # CrC)ation des rankings Mscore
% foo <-transform(foo,  M.rank = rank(-Mscore, ties.method = "first"))
% 
% #tri des donnC)es (on evite les ex-aequo)
% foo <-  foo[with(foo, order(-Ascore, -Y)), ]
% # CrC)ation des rankings Alpha
% foo <-transform(foo,  A.rank = rank(-Ascore, ties.method = "first"))
% 
% res <- data.frame(foo[,6:8])
% #p2 <- ggpcp(res,xlab="bgbg",ylab="byby")
% 
%  print(p2 + geom_line() + 
