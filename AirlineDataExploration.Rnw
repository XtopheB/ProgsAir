...
% 13/06/2013: Amelioration des statistqiues sur scores. Graphiques
% 05/08/2013:  version modifi??e avec Steve (passage en log, fichier avec un seul input, test0...)
...
dea.plot.frontier(dataref.y$K, dataref.y$Y, RTS="fdh", add= TRUE, 
...
foo2 <- xtable(spam[1:20,], digits=2, label ="Eff", caption = paste("50 first airlines efficiency, year",annee1,", m=",M.order,", alpha = ", alpha,"(output oriented, 4 inputs), ",nrow(dataref.y1),"airlines (sorted by alpha-scores)."))
...
foo2 <- xtable(spam[1:20,], digits=2, label ="Eff", caption = paste("50 first airlines efficiency, year",annee1,", m=",M.order,", alpha = ", alpha,"(output oriented, 1 input), ",nrow(dataref.y1),"airlines (sorted by alpha-scores)."))
