...
dataref.y1$Mscore <- spam2$output 

#alpha-frontiere
alpha <- 0.95
foo <- alphascore(xref, yref, xeval=xref, yeval=yref, alpha=alpha)
dataref.y1$Ascore <-foo$output
...
spam <- Benchmarking::dea(X=xref, Y=yref, RTS="vrs", ORIENTATION="out")
dataref.y2$DEA <-1/eff(spam)

# M-frontier 
# Scores output-oriented 

spam2 <- ordermscore(xref, yref, xeval=xref, yeval=yref, m=M.order)
dataref.y2$Mscore <- spam2$output 

#alpha-frontiere
foo <- alphascore(xref, yref, xeval=xref, yeval=yref, alpha=alpha)
dataref.y2$Ascore <-foo$output
...
spam <- spam[order(- dataref.y1$DEA, - dataref.y1$Mscore, - dataref.y1$Ascore),]
foo2 <- xtable(spam[,], label ="Eff", caption = paste("Airlines efficiency, year",annee1,", m=",M.order,", alpha = ", alpha,"(output oriented, 4 inputs)."))
...
foo2 <- xtable(spam[,], label ="Eff", caption = paste("Airlines efficiency, year",annee2,", m=",M.order,", alpha = ", alpha,"(output oriented, 4 inputs)."))
...
foo3 <- xtable(table.malm.ww, label ="malmquistWW", caption = "Malmquist idexes (average) decompsed following  SW(1998) et WW (1999) for a choice of successive years")
digits(foo3)[c(2,3)] <- 0
print(foo3)
@
...
Noms<-unique(Mydata$carriername)
Sub<-subset(Workdata, year==2008 & Y >0 & Region == 1 , select = c(Y, K, L, E))

stars(Sub[1:10,], len = 0.8, key.loc = c(1, 1.5), main = "Airlines Y-K-L typology", draw.segments = TRUE, labels=Noms[1:4])
## Attention noms faux !!
