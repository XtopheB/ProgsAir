...
We base our analysis on two type subsets of airlines: The whoel sample and the sample reduced to majors airlines\footnote{major airlines are defined as having an output greater than the third quuartile of $Y$. See section \ref{Majors} }. We choose \Sexpr{annee1} and  \Sexpr{annee2} to be 2 years of reference in our analysis. These values are parameter of the program and may be changed anytime. \\
...
spam <- spam[order(- dataref.y1$Mscore   - dataref.y1$Ascore  - dataref.y1$DEA),]

foo2 <- xtable(spam[,], digits=2, label ="Eff", caption = paste("Airlines efficiency, year",annee1,", m=",M.order,", alpha = ", alpha,"(output oriented, 4 inputs), ",nrow(dataref.y1),"airlines (sorted by M-scores)."))
...
foo2 <- xtable(spam[,], digits=2, label ="Eff", 
               caption = paste("Airlines efficiency, year",annee2,", 
                            m=",M.order,", alpha = ", alpha,"(output oriented, 4 inputs), 
                               ",nrow(dataref.y2),"airlines, (sorted by M-scores)."))
...
\subsection{Contemporaneous efficiencies}
...
boxplot(score.dea.cont[,2]~score.dea.cont[,1], 
        main = "Airlines efficiency measured to contemporaneous DEA-frontier", 
...
<< echo=FALSE, results=hide, fig=TRUE>>=
p <- qplot(y.c, score.dea.cont, data=score.dea.cont, geom=c("boxplot"), 
      fill=y.c, main="Airlines efficiency measured relative to contemporaneous DEA-frontier",
...
boxplot(score.fdh.cont[,2]~score.fdh.cont[,1], 
        main = "Airlines efficiency measured to contemporaneous FDH-frontier", 
        sub = paste(" Computed on all available airlines (2002-2008)"),  
        ylab = "Efficiency (output oriented)", 
        xlab = "Years ", outline =FALSE)
abline(h = 1, col = "red",  lwd=2)
...
boxplot(score.m.cont[,2]~score.m.cont[,1], 
        main = "Airlines efficiency measured to contemporaneous order-m frontier", 
        sub = paste(" Computed on all available airlines (2002-2008)"),  
        ylab = "Efficiency (output oriented)", 
        xlab = "Years ", outline =FALSE)
abline(h = 1, col = "red",  lwd=2)
...
boxplot(score.alpha.cont[,2]~score.alpha.cont[,1], 
        main = "Airlines efficiency measured to contemporaneous alpha frontier", 
        sub = paste(" Computed on all available airlines (2002-2008)"),  
        ylab = "Efficiency (output oriented)", 
        xlab = "Years ", outline =FALSE)
abline(h = 1, col = "red",  lwd=2)
...
boxplot(score.dea[,2]~score.dea[,1], 
        main = "Airlines efficiency measured to pooled DEA- frontier", 
...
boxplot(score.fdh[,2]~score.fdh[,1], 
        main = "Airlines efficiency  measured to pooled FDH frontier", 
        ylab = "Efficiency (output oriented)", 
        xlab = "Years ", outline =FALSE)
abline(h = 1, col = "red",  lwd=2)
...
boxplot(score.m[,2]~score.m[,1], 
        main = "Airlines m-efficiency  measured to pooled order-m frontier", 
...
%  VERSION de FEAR non compatible avec R 3.0.0
% Resultats verion pr??cendente..


% latex table generated in R 2.15.1 by xtable 1.7-1 package
% Wed May 08 00:54:40 2013
\begin{table}[ht]
\centering
\begin{tabular}{rrrrrrrr}
  \hline
 & An1 & An2 & Malm & Pure Eff & Scale & Pure Tech & ScaleTech \\
  \hline
1 & 2002 & 2003 & 1.13 & 0.99 & 0.99 & 1.17 & 1.00 \\
  2 & 2003 & 2004 & 1.27 & 0.96 & 0.90 & 1.31 & 1.12 \\
  3 & 2004 & 2005 & 1.04 & 0.86 & 1.06 & 1.23 & 0.97 \\
  4 & 2005 & 2006 & 1.11 & 1.23 & 1.08 & 0.96 & 0.94 \\
  5 & 2006 & 2007 & 1.13 & 1.08 & 0.97 & 1.11 & 1.01 \\
  6 & 2007 & 2008 & 1.30 & 0.96 & 0.98 & 1.06 & 1.01 \\
  7 & 1900 & 1900 &  &  &  &  &  \\
  8 & 2002 & 2008 & 1.85 & 1.03 & 0.95 & 1.96 & 1.01 \\
   \hline
\end{tabular}
\caption{Malmquist indexes (average) decomposed following  SW(1998) et WW (1999) for a choice of successive years}
\label{malmquistWW}
\end{table}
...
<<echo =FALSE, results = tex>>=
# mise en forme du tableau
names(table.malm.ww) <- c("An1", "An2", "Malm", "Pure Eff", "Scale" , "Pure Tech" , "ScaleTech", "N","n1", "n2")
...
for (i in 1:length(x)) {
    ...
    score.DEA<- rbind(score.DEA, foo) # Rbind parce que pas le même nombre de firmes...
