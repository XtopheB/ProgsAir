...
\subsection{A different way of defining efficiency scores) }

\frame[t] {
\begin{itemize}[<+->]
 \item   For any $(x,y) \in \Psi$   $\Psi = \left\{ \left( x,y\right) |\;x\; can \; produce \; y \right\}$
 \item $\Psi$ is also the support of $H_{X,Y}(x,y)= Prob( X  \leq x , Y \geq y)$ which can be interpreted as the probability for a firm operating at level (x,y) to be dominated.
  \item Note that :
  \begin{eqnarray*}
   H_{X,Y}(x,y) &=& Prob( X  \leq x | Y \geq y) Prob(Y \geq y )  = F_c(x|y)S_Y(y) \\
                 &=& Prob(Y \geq y | X  \leq x ) Prob(X  \leq x )  = S_c(y|x)F_X(x) \\
   \end{eqnarray*}
   The support of $F_c(x|y)$
\end{itemize}
}
...
\frame[t] {
\begin{itemize}[<+->]
 \item  The production set is defined simply as:  $\Psi = \left\{ \left( x,y\right) |\;x\; can \; produce \; y \right\}$
 \item $\Psi$ is also the support of $H_{X,Y}(x,y)= Prob( X  \leq x , Y \geq y)$ which can be interpreted as the probability for a firm operating at level (x,y) to be dominated.
  \item Note that :
  \begin{eqnarray*}
   H_{X,Y}(x,y) &=& Prob( X  \leq x | Y \geq y) Prob(Y \geq y )  = F_c(x|y)S_Y(y) \\
...
\frame[t] {
\begin{itemize}[<+->]
  \item[order-m] The production set can be described in terms of its sections. $ \forall x$, the frontier in the output direction $\Psi(x) = \{ y | (x,y) \in \Psi \}$. It is also the upper boundary of the support of $F_c(y|x)$. 
\begin{eqnarray*}
 \Psi(x)            &=& sup \left\{ y | F_c(y|x) < 1 \right\} \\
\end{eqnarray*}
\item  $\Psi(x)$ is the maximum level of output attainable for any firm using less than the level $x$ of input.
\item given a fixed integer $m$,  for a given level of input $x$, the  order-m frontier in the output direction $\Psi_m(x)$ is:
\begin{eqnarray*}
 \Psi_m (x)            &=& E \left [ max(Y^1, \cdots, Y^m) | X  \leq x   \right ] \\
\end{eqnarray*}
...
\begin{itemize}[<+->]
  ...
  \item[] $\rightarrow$ Using decomposition of productivity index (Simar-Wilson 98, Wheelock Wilson,  1999 or  Färe, Grosskopft \& Sickles 2007))
...
\begin{itemize}[<+->]
   %\item \textbf{Technical efficiency scores} $\rightarrow$ {\color{brique} Production possibilities set}
    ...
    \begin{equation*}
        \Psi=\left\{ \left( x,y\right) |\;x\; can \; produce \; y \right\}
    \end{equation*}
    \item Nonparametric data envelopment techniques are used to estimate this set. For example, DEA.
    \begin{equation*}
        \widehat{\Psi}_{DEA} = \left\{ \left( x,y\right) |  \;y\leq \sum_{i =1}^n \gamma_{i} Y_{i},x\geq \sum_{i =1}^n  \gamma_{i} X_{i}, \gamma_{i }\geq 0 ; \sum_{i =1}^n \gamma_{i} =1 \right\}
        \end{equation*}
...
% Sur le graphique, on a l'impression que la productivité baisse en 2008 (version DEA) et pas sur order-m (à voir)
